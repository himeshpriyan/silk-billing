...
const ReturnsExchange: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const mockBill: Bill = {
                id: 'bill-123',
                billNo: 'INV-2024-0042',
                customerName: 'Ananya Sharma',
                customerMobile: '9876543210',
                date: '2024-03-20',
                grandTotal: 15500,
                items: [
                    { barcode: 'KS-2023-089', name: 'Pure Silk Saree - Maroon', qty: 1, returnedQty: 0, sellingPrice: 15000, total: 15000 },
                    { barcode: 'ACC-001', name: 'Gift Packing', qty: 1, returnedQty: 0, sellingPrice: 500, total: 500 }
                ]
            };

            if (searchQuery.includes('42') || searchQuery.includes('9876')) {
                setSelectedBill(mockBill);
                setReturnItems(mockBill.items.map((item: BillItem) => ({
                    ...item,
                    returnQty: 1,
                    condition: 'good',
                    isVerified: false,
                ...
                toast.success('Bill found (Mock)');
            } else {
                toast.error('No bill found with this number or mobile (Mock)');
            }
        } catch (error) {
    ...
    const handleProcessReturn = async () => {
        ...
        try {
            ...
            toast.success('Return processed successfully (Mock)');
