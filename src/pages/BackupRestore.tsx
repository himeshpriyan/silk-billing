...
export const BackupRestore: React.FC = () => {
    const { sarees, bills, suppliers, expenses } = useData();
    ...
    const fetchBackupData = async () => {
        try {
            ...
            // Construct mock backup data from local state
            return {
                bills: bills || [],
                inventory: sarees || [],
                customers: bills ? bills.map(b => ({ name: b.customerName, mobile: b.customerMobile, place: b.customerPlace, type: b.customerType })) : [],
                expenses: expenses || [],
                meta: {
                    counts: {
                        bills: bills?.length || 0,
                        customers: bills ? new Set(bills.map(b => b.customerMobile)).size : 0,
                        inventory: sarees?.length || 0,
